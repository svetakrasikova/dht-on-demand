...
-------------------------------------------------------------------------------
	...
	--ranks the set of nodes from the point of view of the node with the specified id based on their distance on the ring
	rank = function(n, set)
		local distances = {}
		local ranked = {}
		for i,v in ipairs(set) do
			...
			local dist_counter = TMAN.t - dist_clockwise
...
-------------------------------------------------------------------------------
	...
	selectPeer = function()
		...
		--log:print("TMAN.selectPeer", misc.time() - sp, "peer id", partner.id%2^TMAN.t)
		return partner
		end,
